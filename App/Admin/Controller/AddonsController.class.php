...
class AddonsController extends BaseController {
	...
	public function lists() {
		$install_addons = D('Addons')->get_installed_addons();
			 ...
			 ->addNav('未安装插件', U('Addons/not_install'), '')
                         ->addNav('插件应用', U('Addons/manage'), '')
			 ->addListItem('logo', '插件LOGO', 'image', array('attr'=>'width=20,height=20'))
	...
	public function not_install() {
		...
		if ($handle) {
			...
			$this ->addNav('设计新插件', U('Addons/add'), '')
                         ->addNav('已安装插件', U('Addons/lists'), '')
			 ->addNav('未安装插件', '', 'active')
                         ->addNav('插件应用', U('Addons/manage'), '')
			 ->addListItem('logo', '插件LOGO', 'image', array('attr'=>'width=15,height=15'))
			 ->addListItem('name', '插件名称')
			 ->addListItem('bzname', '插件标识名')
			 ->addListItem('desc', '插件描述')
			 ->addListItem('version', '当前版本')
...
str;
	...
	public function quote(){
        ...
        $page = I('p') ? I('p') : 1;
        if($search){
			$condition['addons_name'] = array('like',"%".$search."%");
		}else{
			$condition = "";
		}
        $data = pagination($page,'addons_quote',$condition,25);
        // print_r($data);die;
	...
	public function quote_add(){
		if(!IS_POST){
			...
			$res = M('addons_quote')->add($formdata);
			if($res){
				$this->redirect('quote');
			}else{
				$this->error('添加失败');
	...
	public function quote_edit(){
		if (IS_POST) {
			$data = I('post.');
			$res = M('addons_quote')->where('id = '.$data['id'])->save($data);
			if ($res) {
				$this->redirect('quote');
			}else{
				$this->error('更新失败');
			...
			$data = M('addons_quote')->where('id = '.$_GET['id'])->find();
			$this->assign('data',$data);
			$this->display();
		}
	}
	public function quote_mess(){
		$id = I('id');
		$mess_data = M('addons_quote')->where("id=$id")->find();
		$this->assign('data',$mess_data);
		$this->display();
	}
	public function quote_del(){
		$id = I('id');
		$res = M('addons_quote')->where("id=$id")->delete();
		if($res){
			$data['status'] = 1;
			$data['info'] = '删除成功';
		}else{
	...
	public function quote_delall(){
		$id = I('id');
		$lengths = strlen($id);
		$val = substr($id,0,lengths-1);
		// echo $val;die;
		$res = M('addons_quote')->delete($val);
		if($res){
			$data['status'] = 1;
			$data['info'] = '删除成功';
		}else{
